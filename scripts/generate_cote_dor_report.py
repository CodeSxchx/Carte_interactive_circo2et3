...
def build_html(
    *,
    ...
    parts.append("</ul>")
    parts.append("<details class=\"toc\" id=\"tocDetails\">")
    ...
    parts.append("}")
    parts.append("function showMap(){sections.forEach(s=>s.classList.remove('active')); if(viewer) viewer.style.display='none'; if(mapView) mapView.style.display='block'; if(select) select.value='carte';}")
    parts.append("function showCommune(id){sections.forEach(s=>s.classList.toggle('active', s.id===id)); if(mapView) mapView.style.display='none'; if(viewer) viewer.style.display='block'; if(select) select.value=id;}")
    ...
    parts.append("if(toc && !window.matchMedia('(max-width: 860px)').matches){toc.setAttribute('open','');}")
